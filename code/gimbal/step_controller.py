...
class GimbalStepController(GimbalController):
    ...
    def step_move_by_data(self, mode, **kwargs):
        ...
        # 현재 각도(degree)로부터 현재 duty 계산
        current_duty = (self.current_degree / 18.0) + 2.5

        if direction == 1:
            # 시계방향으로 12.5(상대각도 90도)까지 0.1씩 조정
            target_duty = 12.5
            while current_duty < target_duty:
                current_duty = min(current_duty + 0.1, target_duty)
            ...
            target_duty = 2.5
            while current_duty > target_duty:
                current_duty = max(current_duty - 0.1, target_duty)
                self.yaw_pwm.ChangeDutyCycle(current_duty)
                time.sleep(0.02)

        # 최종 물리 각도 갱신
        self.current_degree = (current_duty - 2.5) * 18.0
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"복귀 완료 - 현재 물리 각도: {controller.current_degree:.2f}도")
        time.sleep(1.0)
